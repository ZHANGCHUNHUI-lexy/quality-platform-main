...
function doeResidualCharts(
  ...
  return [
    svgChart('残差图 · 正态概率图', <NormalProbPlot T={T} data={values} h={280} xLabel={residualLabel} xDomain={scale.domain} xTicks={scale.ticks} showPointLabels />, 960, 280),
    ...
    svgChart('残差图 · 与拟合值', <ScatterPlot T={T} xs={fitValues} ys={values} slope={0} intercept={0} xLabel="拟合值" yLabel={residualLabel} h={280} showPointLabels />, 960, 280),
    svgChart('残差图 · 与顺序', <ScatterPlot T={T} xs={orderValues} ys={values} slope={0} intercept={0} xLabel="观测值顺序" yLabel={residualLabel} h={280} showPointLabels />, 960, 280),
  ];
}
...
function buildParetoPayload(): AnalysisReportPayload {
  ...
  return {
    ...
    charts: [svgChart('帕累托图', <ParetoChart T={T} rows={report.rows.map((row) => ({ name: row.name, count: row.count }))} title={app.paretoChartTitle.trim() || `${hasImportedData ? model.name : '示例数据'} 的 Pareto 图`} />, 960, 300)],
  };
}
