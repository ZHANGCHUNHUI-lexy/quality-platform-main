...
interface NormalProbPlotProps {
  ...
  /** 单独诊断图显示每点的横坐标与累计百分比；四合一总览保持简洁。 */
  showPointLabels?: boolean;
}

function NormalProbPlotImpl({ T, data, h, xLabel, xDomain, xTicks, showPointLabels = false }: NormalProbPlotProps) {
  const W = 960;
  const H = h ?? 320;
  ...
  const pointIndexes = showPointLabels ? decimateUniform(pts.length, Math.min(MAX_RENDER_PTS, 40)) : [];
  return (
    <Svg w={W} h={H}>
      <rect x={0} y={0} width={W} height={H} fill={T.bg} />
      {PCT_TICKS.map((p) => {
      ...
      {pointIndexes.map((i) => (
        <Txt key={'label' + i} x={X(pts[i].x) + 6} y={Y(pts[i].z) - 7} s={`(${nf(pts[i].x, xDp)}, ${nf(((i + 0.5) / pts.length) * 100, 1)}%)`} fill={T.text} size={9} />
      ))}
