    ...
    <Svg w={W} h={H}>
      ...
      {rows.map((r, i) => {
        ...
        return (
          <Fragment key={'b' + i}>
            ...
            />
            <Txt x={(x0 + x1) / 2} y={Yc(r.count) - 8} s={String(r.count)} fill={classic ? '#111111' : T.text} size={10} anchor="middle" weight={600} />
      ...
      {cumPts.map((pt, i) => (
        <Fragment key={'c' + i}>
          ...
          <Txt x={pt[0]} y={pt[1] - 11} s={pt[2] + '%'} fill={cumLine} size={9.5} anchor="middle" weight={600} />
        </Fragment>
      ))}
      {showTable ? (
