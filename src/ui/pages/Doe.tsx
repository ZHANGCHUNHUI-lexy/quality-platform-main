    ...
    <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
      ...
      {!isDemo && (() => {
        ...
        const paneChart = (pane: 'normal' | 'fits' | 'hist' | 'order', height: number) =>
          pane === 'normal' ? <NormalProbPlot T={T} data={rr} h={height} xLabel={yl} xDomain={residualScale.domain} xTicks={residualScale.ticks} showPointLabels={residLayout === 'single'} />
            : pane === 'fits' ? <ScatterPlot T={T} xs={fitsF} ys={rr} slope={0} intercept={0} xLabel="拟合值" yLabel={yl} h={height} showPointLabels={residLayout === 'single'} />
              : pane === 'hist' ? <MiniHist T={T} data={rr} h={height} title={`${yl}分布`} xLabel={yl} xDomain={residualScale.domain} xTicks={residualScale.ticks} />
                : <ScatterPlot T={T} xs={runOrder} ys={rr} slope={0} intercept={0} xLabel="观测值顺序" yLabel={yl} h={height} showPointLabels={residLayout === 'single'} />;
          ...
          <>
            ...
            <Card>
              ...
              <div style={{ padding: '10px 12px 12px' }}>
                {rr.length === 0
                  ? <div style={{ padding: 20, color: '#9aa2ad', fontSize: 12.5 }}>当前残差类型下没有可绘制的有限值。</div>
                  : <ScatterPlot T={T} xs={factorVals} ys={rr} slope={0} intercept={0} xLabel={`${design!.factorNames[rf]}(实际水平)`} yLabel={yl} h={240} showPointLabels />}
              </div>
            </Card>
          </>
        );
