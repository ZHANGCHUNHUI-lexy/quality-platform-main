      ...
      <Card>
        ...
        {residView === 'four' ? (
          ...
          <div style={{ padding: '10px 12px 12px' }}>
            {residView === 'normal' && <NormalProbPlot T={T} data={resid.residuals} h={340} showPointLabels />}
            {residView === 'fits' && <ScatterPlot T={T} xs={resid.fits} ys={resid.residuals} slope={0} intercept={0} xLabel="拟合值(组均值)" yLabel="残差" h={340} showPointLabels />}
            {residView === 'hist' && <MiniHist T={T} data={resid.residuals} h={340} xLabel="残差分布" />}
            {residView === 'order' && <ScatterPlot T={T} xs={resid.residuals.map((_, i) => i + 1)} ys={resid.residuals} slope={0} intercept={0} xLabel={residualsInWorksheetOrder ? '观测序(工作表行序)' : '观测序(按组)'} yLabel="残差" h={340} showPointLabels />}
          </div>
        )}
      </Card>
    </div>
...
function ParetoInner({ T }: { T: ChartTokens }) {
  const {
    ...
    paretoThreshold,
    paretoChartTitle, setParetoOptions,
  ...
  const defaultTitle = `${isReal ? pareto!.name : '示例数据'} 的 Pareto 图`;
  const chartTitle = paretoChartTitle.trim() || defaultTitle;
  ...
  return (
    <div style={{ display: 'grid', gridTemplateColumns: '1fr 300px', gap: 16 }}>
      <Card>
        ...
        />
        <label style={{ display: 'flex', alignItems: 'center', gap: 8, padding: '10px 16px 0', fontSize: 12, color: '#5b6472' }}>
          图表标题
          <input aria-label="帕累托图表标题" value={paretoChartTitle} placeholder={defaultTitle}
            onChange={(event) => setParetoOptions({ paretoChartTitle: event.target.value })}
            style={{ flex: 1, minWidth: 0, border: '1px solid #d9dee5', borderRadius: 4, padding: '5px 7px', font: 'inherit' }} />
        </label>
        <div style={{ padding: '14px 16px 6px' }}>
          <ParetoChart T={T} rows={display} w={960} h={340} title={chartTitle} />
        </div>
      </Card>
      <Card>
