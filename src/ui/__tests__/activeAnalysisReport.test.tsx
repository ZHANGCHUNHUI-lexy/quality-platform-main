...
describe('buildActiveAnalysisReport', () => {
  it('MSA 报告只使用用户填写的量具信息，缺失值不再伪造成操作员或当天日期', () => {
    ...
    expect(figure).toMatch(/<svg[^>]*width="\d+"[^>]*height="\d+"/);
  ...
  it('帕累托图的自定义标题同步到专项报告', () => {
    useData.getState().importPareto('标题测试', [{ name: '不良A', count: 3 }, { name: '不良B', count: 2 }]);
    useApp.setState({ page: 'pareto', paretoChartTitle: '产线 A 缺陷帕累托图' });
    const report = buildActiveAnalysisReport();
    expect(report?.charts.find((chart) => chart.title === '帕累托图')?.svg).toContain('产线 A 缺陷帕累托图');
  });
