...
interface AppState {
  ...
  paretoChartTitle: string; // 用户可编辑的图内标题；空值时按数据名称生成默认标题
  ...
  setParetoOptions(patch: Partial<Pick<AppState, 'paretoMergeOther' | 'paretoThreshold' | 'paretoChartTitle'>>): void;
...
function persistedPrefsOf(s: AppState) {
  return {
    ...
    paretoChartTitle: s.paretoChartTitle,
  };
}
...
export const useApp = create<AppState>()(persist((set, get) => ({
  ...
  setParetoOptions: (patch) => set((s) => {
    ...
    const paretoChartTitle = typeof patch.paretoChartTitle === 'string'
      ? patch.paretoChartTitle.slice(0, 120)
      : s.paretoChartTitle;
    return { ...patch, paretoThreshold, paretoChartTitle };
  }),
  ...
  merge: (persisted: unknown, current) => {
    ...
    merged.paretoChartTitle = typeof p.paretoChartTitle === 'string' ? p.paretoChartTitle.slice(0, 120) : current.paretoChartTitle;
...
export function resetUiPreferences(): void {
  useApp.setState({
    ...
    paretoView: 'pareto', paretoMergeOther: false, paretoThreshold: 0.95, paretoChartTitle: '',
  });
}
